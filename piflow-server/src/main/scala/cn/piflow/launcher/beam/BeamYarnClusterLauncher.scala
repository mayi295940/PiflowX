...
object BeamYarnClusterLauncher {
  ...
  def launch[DataType](flow: Flow[DataType]): String = {
    ...
    val clusterSpecification = new ClusterSpecification.ClusterSpecificationBuilder().createClusterSpecification
    // 设置用户jar的参数和主类
    ...
    val yarnClusterDescriptor = new YarnClusterDescriptor(flinkConfiguration, yarnConfiguration, yarnClient, clusterInformationRetriever, true)
    var clusterClientProvider: ClusterClientProvider[ApplicationId] = null
    try clusterClientProvider = yarnClusterDescriptor.deployApplicationCluster(clusterSpecification, appConfig)
    catch {
