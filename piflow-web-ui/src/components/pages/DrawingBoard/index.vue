    ...
    >
      <div :style="{ height: `${Programming}` }">
        <RadioGroup size="small" v-model="buttonSize" type="button">
          <Radio label="text" :disabled="buttonSize === 'text' ? false : true"
            >text</Radio
          >
          <Radio label="scala" :disabled="buttonSize === 'scala' ? false : true"
            >scala</Radio
          >
          <Radio
            label="javascript"
            :disabled="buttonSize === 'javascript' ? false : true"
            >java</Radio
          >
          <Radio
            label="python"
            :disabled="buttonSize === 'python' ? false : true"
            >python</Radio
          >
          <Radio label="sh" :disabled="buttonSize === 'sh' ? false : true"
            >shell</Radio
          >
          <Radio label="sql" :disabled="buttonSize === 'sql' ? false : true"
            >sql</Radio
          >
        </RadioGroup>
        <code-editor
...
export default {
  ...
  methods: {
        ...
        .then((res) => {
          ...
          if (dataMap.code == 200) {
            document
              ...
              .contentWindow.document.getElementById(`${this.stopsId}`).value =
              dataMap.value;
            document
              .getElementById("bariframe")
    ...
    JumpToMonitor(id, msg) {
      ...
      document
        .getElementById("bariframe")
        .contentWindow.postMessage(pageURl, "*");
