...
import cn.piflow.bundle.spark
import cn.piflow.bundle.spark.util.StringUtil
import cn.piflow.spark.util
  ...
  def applyText (set: Map[Hierarchy, (String, String)]) : Unit = {
    for (pair <- hiMap) {
      hiMap = hiMap.updated(pair._1, {
        ...
        new KeywordStatus(pair._2.keywordFrequency, StringUtil.countString(absTitle._1, name),
          spark.util.StringUtil.countString(absTitle._2, name), amount, true)
      })
    }
  }
