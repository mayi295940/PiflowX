...
class MysqlCdc extends ConfigurableStop[Table] {
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val url = new PropertyDescriptor()
      ...
      .description("MySQL 数据库服务器的IP地址或主机名。")
      .defaultValue("")
      .required(true)
      .example("127.0.0.1")
    ...
    val tableName = new PropertyDescriptor()
      ...
      .description("需要监视的MySQL数据库的表名。表名支持正则表达式，以监视满足正则表达式的多个表。" +
        "注意：MySQL CDC 连接器在正则匹配表名时，会把用户填写的 database-name， table-name 通过字符串 `\\\\.` " +
        "连接成一个全路径的正则表达式，然后使用该正则表达式和 MySQL 数据库中表的全限定名进行正则匹配。。")
      .defaultValue("")
      .required(true)
      .language(Language.Text)
      .example("test")
