 ...
 * @author mayi
 */
public class PipelineRefDeletionOperation
    extends PipelineDevelopmentOperation<RefJobContentRequestRefImpl, ResponseRef>
    implements RefDeletionOperation<ThirdlyRequestRef.RefJobContentRequestRefImpl> {

  private static final String DELETE_RULE_URL = "/pipeline/outer/api/v1/projector/rule/delete";
  private static final Logger LOGGER = LoggerFactory.getLogger(PipelineRefDeletionOperation.class);
  ...
  @Override
  public ResponseRef deleteRef(RefJobContentRequestRefImpl requestRef)
    ...
    try {
      ...
      throw new ExternalOperationFailedException(
          90156, "Construct delete outer url failed when delete.");
    ...
    if (jobContent == null
        || !jobContent.containsKey("ruleGroupId")
        || jobContent.get("ruleGroupId") == null) {
    ...
    try {
      ...
      LOGGER.info(
          "Start to delete rule. url: {}, method: {}, body: {}",
          String.format("%s/%s", url, DELETE_RULE_URL),
      ...
      if (status != null && HttpStatus.OK.value() == Integer.parseInt(status)) {
        ...
        throw new ExternalOperationFailedException(
            90156, "Delete outer url return status is not ok.");
      }
    } catch (RestClientException e) {
      LOGGER.error("Failed to delete rule because of restTemplate exception. Exception is: {}", e);
      throw new ExternalOperationFailedException(
          90156, "Delete outer url request failed with rest template.");
    }
    return ResponseRef.newExternalBuilder().success();
  }
}
