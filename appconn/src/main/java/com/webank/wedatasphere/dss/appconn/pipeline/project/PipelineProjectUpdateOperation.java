 ...
 * @author mayi
 */
public class PipelineProjectUpdateOperation
    extends AbstractStructureOperation<ProjectUpdateRequestRefImpl, ResponseRef>
    implements ProjectUpdateOperation<ProjectUpdateRequestRef.ProjectUpdateRequestRefImpl> {

  private static final Logger LOGGER = LoggerFactory.getLogger(PipelineProjectUpdateOperation.class);

  private static final String UPDATE_PROJECT_PATH = "pipeline/outer/api/v1/project/workflow";
  ...
  public ResponseRef updateProject(ProjectUpdateRequestRef.ProjectUpdateRequestRefImpl projectRef)
    ...
    try {
      ...
      LOGGER.error("Update pipeline Project Exception", e);
      throw new ExternalOperationFailedException(
          90176, "Update pipeline project by build url exception", e);
    ...
    try {
      ...
      LOGGER.error("Update pipeline Project Exception", e);
      throw new ExternalOperationFailedException(90176, "Update pipeline project exception", e);
    }
    ...
    Integer projectId =
        (Integer)
            ((Map<String, Object>) ((Map<String, Object>) resMap.get("data")).get("project_detail"))
                .get("project_id");
    LOGGER.info("Update pipeline project ID: {}", projectId);
